...
import pygame
import sys
from support import WHITE, BACKGROUND_COLOR, LIGHT_GREY, GAME_LEVELS, screen_height, screen_width, sidebar_width
from game_page import draw_button
...
def draw_home_screen(screen, stats):
    ...
    for i, (level, info) in enumerate(GAME_LEVELS.items()):
        ...
        pygame.draw.rect(screen, LIGHT_GREY, (button_x, button_y, button_width, button_height))
        ...
        text_render = font_small.render(level_text, True, WHITE)
    ...
    # Draw "STATS" heading
    stats_heading_font = pygame.font.SysFont(None, 48)
    stats_heading_text = "STATS"
    stats_heading_render = stats_heading_font.render(stats_heading_text, True, WHITE)
    screen.blit(stats_heading_render, (sidebar_x + (sidebar_width - stats_heading_render.get_width()) // 2, 20))
    
    stats_start_y = 100
    for i, level in enumerate(GAME_LEVELS):
        stats_text_y = stats_start_y + i * 100
        level_render = font_small.render(level.capitalize(), True, WHITE)
        ...
        if level in stats:
            ...
            stats_render = font_stats.render(stats_text, True, WHITE)
            best_time_render = font_stats.render(best_time_text, True, WHITE)
    ...
    quit_text = "QUIT"
    quit_render = font_small.render(quit_text, True, WHITE)
